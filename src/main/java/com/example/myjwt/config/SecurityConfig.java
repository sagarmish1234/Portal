...
import com.example.myjwt.security.JwtAuthenticationEntryPoint;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.cors().and().csrf().disable()
				...
				.permitAll().antMatchers("/h2-console/**","/api/auth/**").permitAll()
				.antMatchers("/api/welcome/all","/api/welcome/all/story","/api/welcome","/api/all").permitAll()
				.antMatchers(HttpMethod.GET, "/api/users/**").permitAll().anyRequest()
