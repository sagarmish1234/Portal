...
import "../css/pyramidbypractice.css";
import GradeMapping from "../common/GradeMapping";

const PyramidByPractice = () => {
  ...
  useEffect(() => {
    refreshPyramidData();
  }, [practiceChecked, projectTypeChecked, projectOwningChecked]);
  ...
  const refreshPyramidData = () => {
    ...
    assignmentData.map((associate) => {
      if (practiceChecked.indexOf(associate.serviceLine) > -1) {
        ...
        totalMap = getOrCreateChildMap(serviceLineMap, keyTotal);

        let gradeIdx = GradeMapping.getGradePos(associate.gradeDescription);
        gradeMap = getOrCreateChildMap(serviceLineMap, gradeIdx);
  ...
  const renderFTEData = (gradeKey, itemGrade, totalMap) => {
    return (
      <>
        ...
        <td className="gridviewcontentsubtotal">
          {toPercentage(
            toNumber(itemGrade.get(keyOffshoreBill)) +
              ...
              toNumber(itemGrade.get(keyOffshoreNBL)) +
              toNumber(itemGrade.get(keyOnsiteNBL)),
            toNumber(totalMap.get(keyOffshoreBill)) +
              toNumber(totalMap.get(keyOnsiteBill)) +
              toNumber(totalMap.get(keyOffshoreNBL)) +
              toNumber(totalMap.get(keyOnsiteNBL))
          )}
        </td>
      </>
    );
  ...
  const renderTableData = () => {
    ...
    pyramidData.forEach((itemObjSL, pyramidKey, pyramidData) => {
      totalMap = itemObjSL.get(keyTotal);
      rowSpanLen = itemObjSL.size;
      itemObjGradeSorted = new Map([...itemObjSL.entries()].sort());

      itemObjGradeSorted.forEach((itemGrade, gradeKey, itemObjGradeSorted) => {
...
export default PyramidByPractice;
